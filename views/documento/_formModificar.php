...
?>
	...
	<div class="box-body">
		<?php $form = ActiveForm::begin([
			...
			<?php $model->cedulaSolicitante = $modelSolicitante->CEDULA; ?>
			<?php $nacionalidad = Html::activeDropDownList($model,'nacionalidadSolicitante',['E'=>'E','V'=>'V'],['class'=>'form-control','style'=>'width:55px;','disabled'=>'disabled']); ?>
			<?php echo $form->field($model, 'cedulaSolicitante', [
				'inputTemplate' => '<div class="input-group"><span class="input-group-addon" style="padding:0;border:0px;">'.$nacionalidad.'</span>{input}</div>',
			])->textInput(['maxlength'=>'9','disabled'=>'disabled']); 
			?>
			
				<div id="cargando"></div>
				<!-- DATOS DEL SOLICITANTE -->
				<div id="div_datos_persona" >
						<div class="box box-danger" style="margin-bottom:0;">
							...
							<div class="box-body">
								...
								<?= Html::hiddenInput('movimiento', $movimiento); ?>
								
								<?= $form->field($modelSolicitante, 'PRIMER_NOMBRE')->textInput(['maxlength' => true,'disabled'=>true]) ?>

								<?= $form->field($modelSolicitante, 'SEGUNDO_NOMBRE')->textInput(['maxlength' => true,'disabled'=>true]) ?>
							
								<?= $form->field($modelSolicitante, 'PRIMER_APELLIDO')->textInput(['maxlength' => true,'disabled'=>true]) ?>

								<?= $form->field($modelSolicitante, 'SEGUNDO_APELLIDO')->textInput(['maxlength' => true,'disabled'=>true]) ?>
								...
								<?php $codigoTelefono = Html::activeDropDownList($modelSolicitante,'COD_TELEFONO',['0212'=>'0212','0416'=>'0416','0426'=>'0426'],['class'=>'form-control','style'=>'width:75px;','disabled'=>'disabled']); ?>
								...
								<?= $form->field($modelSolicitante, 'CORREO_ELECTRONICO')->textInput(['maxlength' => true,'disabled'=>'disabled'])/*->widget(yii\widgets\MaskedInput::className(), ['mask' => '*{1,20}[.*{1,20}][.*{1,20}][.*{1,20}]@*{1,20}[.*{2,6}][.*{1,2}]',])*/ ?>
							</div>
					</div>
				</div>
				<!-- DATOS DEL SOLICITANTE -->
			...
			<?= $form->field($model, 'NUM_DOCUMENTO', ['enableAjaxValidation' => true])->textInput(['disabled' => true]) ?>

			<?= $form->field($model, 'ID_TIPO_DOCUMENTO')->dropDownList(
				ArrayHelper::map(TIPODOCUMENTO::find()->All(), 'ID_TIPO_DOCUMENTO','DESCRIPCION'),
				['prompt'=>'Seleccione','disabled'=>'disabled']
			)?>
    ...
    <div class="box-footer text-center">
        <?= Html::submitButton($model->isNewRecord ? 'Crear Trámite' : 'Actualizar', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary', 'id'=>'botonSubmit']) ?>
    </div>
